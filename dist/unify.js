(function (global, factory) {
  ...
  function unify(input, options = {}) {
    ...
    while (cwdPathPrefix) {
      ...
      text = text.replace(re, cwdReplacement); // execute the equivalent of path.dirname(), without requiring the `path` module:
    ...
    if (options.reducePaths) {
      ...
      for (let i = 0, len = arr.length; i < len; i++) {
        let el = arr[i];
        if (!el) continue; // decode spec item: {id, replacement}
